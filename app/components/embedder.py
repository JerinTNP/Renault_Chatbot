...
#neew logic - fixing rate limit error while embedding - page by page
"""
Data Extraction Module
The module is for data extraction operations
this module will extract texts from pdf,documents and powerpoint files , embedd them and store in vector database
...
class DATA_LOADER_SP():
    ...
    async def download_file_to_temp(
        ...
        try:
            ...
            # Download the PDF file content into the stream
            await asyncio.to_thread(
                self.ctx.web.get_file_by_server_relative_url(server_relative_url).download(doc_stream).execute_query
            )
            ...
            temp_file_path = os.path.join(temporary_directory, file_name)
            async with aiofiles.open(temp_file_path, "wb") as temp_file:
                await temp_file.write(doc_stream.getvalue()) 
    ...
    async def convert_pdf_to_text(
        ...
        *** MODIFIED TO RUN SEQUENTIALLY TO AVOID AZURE RATE LIMITS ***
        """
        try:
            documents = []
            embed_flag = False
            ...
            # --- FIX START: SEQUENTIAL PROCESSING ---
            if args_list:
                extractor = IMAGE_EXTRACTION()
                # We loop through each page one by one instead of asyncio.gather
                for args in args_list:
                    try:
                        # Await the result of a single page before moving to the next
                        result = await extractor.process_image(args)
                        documents.append(result)
                        # Optional: Add a tiny sleep if you still hit limits
                        # await asyncio.sleep(0.5) 
                    except Exception as e:
                        logging.error(f"Failed to process a page in sequence: {e}")
    ...
    def list_all_files_in_folder_sp(
        ...
        try:
            ...
            def list_files_in_folder(folder):
                folder_files = folder.files.get().execute_query() 
                folder_folders = folder.folders.get().execute_query()
                ...
                for file in folder_files:
                    ...
                    modified_date_list.append(modified_date)
                    full_path = f"{self.sharepoint_url}{server_relative_url}"
                    file_list.append(file_name)
                    file_path_list.append(full_path)
                    linkuri = file.properties["LinkingUri"]
                    if linkuri == None:
    ...
    async def load_and_split_documents_sp(
        ...
        """Asynchronously loads a SharePoint document, processes it, and stores its embeddings."""
        try:
            with db_info.get_db_context() as db: 
                temporary_directory = "temp_dir"
                if not os.path.exists(temporary_directory):
                ...
                if file_path.lower().endswith((".doc", ".docx", ".ppt", ".pptx", ".pdf")):
                    ...
                    try:
                        ...
                        if documents and embedded_flag:
                            ...
                            if fileinfo:
                                fileinfo.embed_done = embedded_flag
                                db.commit()
                                db.refresh(fileinfo)
                            else:
                                logging.error(f"db_info.FileInfo not found for : {file_path}")
    ...
    async def data_to_db_sp(
            ...
            source: str,
            gu_id: str = None):
        """Asynchronously processes and adds SharePoint files to vectorstore."""
        try:
            with db_info.get_db_context() as db:
