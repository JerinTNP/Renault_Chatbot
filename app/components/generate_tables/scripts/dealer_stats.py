...
def get_dealer_stats(data_high_level, data_detailed, digital_data, digital_table,report_type):
    """
    ...
    try:
        ...
        if report_type == "dacia-v1":
            ...
            dealer_stat['journey_experience_dacia'] = detect_stats_detailed("JOURNEY", digital_data,'left',occurrence=2) #with dacia
            dealer_stat['website_conformity_dacia'] = detect_stats_detailed("Website conformity", digital_data,'right')#with dacia
            # dealer_stat['digital_score'] = detect_stats_below("Digital Score", digital_data) #with dacia
            temp_val = detect_stats_below("Digital Score", digital_data)
            dealer_stat['digital_score'] = temp_val
