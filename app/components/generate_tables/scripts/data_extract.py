...
def extract_data(file_path):
    ...
    try:
        ...
        digital_data = (lambda x: pd.Series(pd.to_numeric(x[1:], 'ignore'), name=x[0]))(text_.split('\n')).to_frame()
        ...
        dealer_detailed = (lambda x: pd.Series(pd.to_numeric(x[1:], 'ignore'), name=x[0]))(text.split('\n')).to_frame()  
...
def filter_answers_data(dealer_detailed, config, report_type):
    """
    ...
    """
    try:
        activity_list = config["ACTIVITIES"]
        if report_type == "dacia-v1":
            above_line = "1 - The Dacia facade and dedicated entrance comply with the brand’s visual identity charter"
            digital_line = "201 - The dealer is correctly represented by Google via his GMB profile"
        elif report_type == "dacia-v2":
            above_line = "1 - The Dacia facade and dedicated entrance comply with the brand’s visual identity charter"
            digital_line = "201 - The dealer is correctly represented by Google via his GPB (GMB) profile"
        elif report_type == "renault-v1":
            above_line = "10 - The dealership exterior is in impeccable condition, is clean and is well maintained"
            digital_line = "201 - The dealer is correctly represented by Google via his GMB profile"
        elif report_type == "renault-v2":
            above_line = "10 - The dealership exterior is in impeccable condition, is clean and is well maintained"
            digital_line = "201 - The dealer is correctly represented by Google via his GPB (GMB) profile"
        else:
