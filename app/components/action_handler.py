...
from app.models.prompts import HISTORY_PROMPT, CONTEXT_PROMPT, ROUTE_PROMPT_RENAULT, NOT_ANSWERABLE_PROMPT, SQL_PROMPT , ANALYSIS_PROMPT                                                          
...
def process_analysis_output(output: BaseMessage) -> str:
    """
    Checks if the analysis output is a JSON chart request; if so, returns the JSON string.
    Otherwise, returns the regular text content.
    """
    content = output.content.strip()
   
    # 1. Check for the JSON fence (from the CRITICAL FORMAT rule)
    if content.startswith("```json"):
        logging.info("Chart JSON detected.")
        try:
            # Extract content between the fences
            json_str = content.strip("```json").strip("```").strip()
           
            # Optionally validate/parse the JSON to ensure it's valid
            # If parsing fails, treat it as regular text
            json.loads(json_str)
           
            return json_str
        except json.JSONDecodeError:
            logging.error("Failed to decode detected JSON chart output. Treating as text.")
            pass # Fall through to return content as regular text
           
    # 2. Return content if not a chart JSON
    return content
    ...
    try:
        ...
        def execute_sql(sql: str):
            ...
            try:
                ...
                logging.error(f"SQL execution error: {e}")
                user_error_message = (
                    "I apologize, a technical error occurred while trying to execute the database query. "
                    "Please check your input or rephrase your question."
                )
...
async def create_renault_agent(
    ...
    try:
        ...
        # Crucial for returning a complete state even on failure
                ...
                error_response = f"I encountered a technical error during database analysis: {e}"
...
#             except Exception as e:
#                 logging.error(f"SQL execution error: {e}")
#                 return AIMessage(content=f"I encountered an error while running the query: {e}")
...
#         sql_chain = (
#             {"input": RunnablePassthrough()} # Maps the string input to the {input} key for the prompt
#             | sql_prompt
#             | chat_model
#             | RunnableLambda(get_content_from_response)
#         )
